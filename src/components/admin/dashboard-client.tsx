...
export function DashboardClient({ students }: { students: StudentData[] }) {
  ...
  // Example mock calculation for "Recent Activity" 
  const recentStudents = [...students].sort((a, b) => b.createdAt.getTime() - a.createdAt.getTime()).slice(0, 4)
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm flex flex-col justify-between h-32">
            ...
            <div className="flex items-end justify-between">
              <span className="text-4xl font-bold tracking-tight">{totalRegistrations}</span>
          ...
          <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm flex flex-col justify-between h-32">
            ...
            <div className="flex items-end justify-between">
              ...
              <span className="text-slate-400 text-sm">/ {totalRegistrations} Students</span>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
            ...
            <div className="space-y-6">
              {recentStudents.length === 0 ? (
                <div className="text-sm text-slate-500 text-center py-4">No recent registrations.</div>
              ) : (
                recentStudents.map((item, i) => (
                  <div key={i} className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded-full bg-slate-100 flex items-center justify-center text-xs font-medium text-slate-600 uppercase">
                    ...
                    <div className={`text-xs px-2 py-1 rounded-full font-medium ${
                      item.registrations[0]?.status === 'CONFIRMED' ? 'bg-emerald-50 text-emerald-700' : 'bg-amber-50 text-amber-700'
                    }`}>
                      {item.registrations[0]?.status || 'PENDING'}
                    </div>
                  </div>
                ))
              )}
                      ...
                      <td className="px-6 py-4 font-medium text-slate-900">
                        <div className="flex items-center gap-2">
                          ...
                          {student.registrations[0]?.receivesFreeShirt && (
                            <div className="bg-[#fcdb44]/20 text-yellow-600 p-1 rounded flex items-center gap-1" title={`Free Shirt: ${student.tShirtSize}`}>
                              <Shirt className="h-3.5 w-3.5" />
                              <span className="text-[10px] uppercase font-bold">{student.tShirtSize === "N/A" || student.tShirtSize === "Not Specified" ? "" : student.tShirtSize}</span>
                            </div>
                          )}
                        </div>
                      </td>
                      <td className="px-6 py-4 text-slate-500">{student.age}</td>
                      <td className="px-6 py-4 text-slate-500">{student.parent?.email || "N/A"}</td>
                      <td className="px-6 py-4">
                        <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                          student.registrations[0]?.status === 'CONFIRMED' ? 'bg-emerald-50 text-emerald-700' : 'bg-amber-50 text-amber-700'
                        }`}>
